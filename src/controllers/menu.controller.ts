...
import { generateUniqueSlug, generateUniqueMenuId, validateSlug } from '../utils/slugGenerator';
...
export async function createMenu(req: Request, res: Response): Promise<void> {
  try {
    const userId = req.user!.userId;
    const { nameAr, nameEn, descriptionAr, descriptionEn, slug: customSlug, logo, theme = 'default' } = req.body;
    ...
    const isIdString = columnCheck.recordset.length > 0 && 
...
export async function getMenuById(req: Request, res: Response): Promise<void> {
  try {
    ...
    res.json({ menu: result.recordset[0] });
  } catch (error) {
...
export async function updateMenu(req: Request, res: Response): Promise<void> {
  try {
    const userId = req.user!.userId;
    const { id } = req.params;
    const { nameAr, nameEn, descriptionAr, descriptionEn, logo, theme, isActive } = req.body;
    ...
    await executeTransaction(async (transaction) => {
      ...
      const menuRequest = transaction.request().input('id', sql.Int, parseInt(id));
      ...
      if (nameAr !== undefined || descriptionAr !== undefined) {
        ...
        const arRequest = transaction.request().input('menuId', sql.Int, parseInt(id));
      ...
      if (nameEn !== undefined || descriptionEn !== undefined) {
        ...
        const enRequest = transaction.request().input('menuId', sql.Int, parseInt(id));
...
// Toggle menu status
export async function toggleMenuStatus(req: Request, res: Response): Promise<void> {
  try {
    const userId = req.user!.userId;
    const { id } = req.params;
